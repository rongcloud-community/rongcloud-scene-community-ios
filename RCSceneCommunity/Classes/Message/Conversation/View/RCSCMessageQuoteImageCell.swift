//
//  RCSCMessageQuoteImageCell.swift
//  RCSceneCommunity
//
...
class RCSCMessageQuoteImageCell: RCSCMessageBaseCell {
    ...
    override func updateUI(_ message: RCMessage) -> RCSCCellProtocol {
        ...
        if let path = refMsg.realLocalPath(),
           ...
           let image = UIImage(data: data) {
            quoteContentImageView.image = image
        } else if let thum = refMsg.thumbnailImage {
            quoteContentImageView.image = thum
        } else if let url = refMsg.remoteUrl {
